  ...
  def __init__(self, config,
    ...
    if not label_lookup_path:
      label_lookup_path = os.path.join(
          config['modelDir'], 'imagenet_2012_challenge_label_map_proto.pbtxt')
    if not uid_lookup_path:
      uid_lookup_path = os.path.join(
          config['modelDir'], 'imagenet_synset_to_human_label_map.txt')
...
class Model(object):
  ...
  def predict(self, image_url):
    ...
    node_lookup = NodeLookup(self.config)
    ...
    for node_id in top_k:
      ...
      humanStrings.append(human_string)
    return human_string
